...
bool inint(int inp, std::vector<int> op){
    for (int i = 0; i < op.size() ; i ++){
    if (inp == op[i]){
        return true;
            //Do Stuff
...
class Var {
    ...
    void setallocd(GlobalVariable* a){
...
class lir {
    ...
    std::unordered_map<std::string,Var> gvars;
    ...
    // Pointer type
    typings["ptr"] = builder.getPtrTy();  // Generic pointer (i8*) doesn't actually show
    ...
    typings["void"] = builder.getVoidTy();    // Represents ASCII characters (i8)

    // String type
    typings["RawString"] = PointerType::get(Type::getInt8Ty(context),0); // String represented as i8*
    return typings;
    ...
    mainFunction->setDSOLocal(false);
...
if (&builder == nullptr) {
    ...
    Type* initStruct(std::string name,std::vector<Type*> pts){
        ...
        s->setBody(pts);
        structs[name] = s;
        if (AP.has("-deadcode") == false){ // maybe create deadcode block
    ...
    Value* initStructInstance(AllocaInst* withinfn,std::string name,std::string as,std::unordered_map<int,Value*> fv){
        auto v = withinfn;
        auto vc = Var();
        vc.setallocd(v);
        vars[as] = vc;
...
public:
    ...
    std::string parsecall(){
        ...
        if (is("cmethod")){
            advance();
            auto fnid = consume().value;
            auto expr = parseexp();
            std::string mid;
            if (Lir.fns.find(expr[0] + "cmethod" + fnid) == Lir.fns.end()){
                mid = expr[1] + "cmethod" + fnid; // struct's personal method
            } else {
                
                mid = expr[0] + "cmethod" + fnid;
            }
    ...
    std::string parsemodify(){
        expect("modify");
        if (is("ptr")){
            advance(); // ptr
            auto e = parseexp();
            Lir.builder.CreateStore(getvalue(e[1]),getvalue(e[0]));
            return "";
        }
        if (is("cvalue")){
            advance(); // cvalue
            ...
            if (Lir.gvars.find(e[0] + "cvalue" + id) == Lir.gvars.end()){
                v = Lir.gvars[e[1] + "cvalue" + id];
                lstr = e[1] + "cvalue" + id;


            } else {
                v = Lir.gvars[e[0] + "cvalue" + id];
                lstr = e[1] + "cvalue" + id;
            }
            Lir.builder.CreateStore(getvalue(e[2]),v.gallocd);
            return "";
        }
    }
    ...
    std::string parsesig(){
        ...
        expect(")");
        expect("->");
        std::string rettyp = consume().value;
        std::vector<Type*> t;
        for (int i = 0; i != map.size(); i++){
            t.push_back(gettype(map[i]));
        }
        
        auto fnt = Lir.getfnt(gettype(rettyp),t,vdic);
    ...
    std::string mangle(std::string v,std::string b="", std::string t= "var"){
        ...
        if (manglechain.find(blockchain[blockchain.size()-1]) == manglechain.end()){
            std::unordered_map<std::string,std::string> insert;
            insert[v] = blockchain[blockchain.size()-1] + t + v;
            manglechain[blockchain[blockchain.size()-1]] = insert;
        } else {
            manglechain[blockchain[blockchain.size()-1]][v] = blockchain[blockchain.size()-1] + t + v;
        }
        return blockchain[blockchain.size()-1] + t + v;
            ...
            if (manglechain.find(blockchain[blockchain.size()-1]) == manglechain.end()){
                ...
                manglechain[blockchain[blockchain.size()-1]][v] = b + t + v;
            }
            return b + t + v;
        }
    }
    ...
    std::string parseassignable(){
        ...
        if (is("block")){
            ...
            if (last == ""){
                throw std::runtime_error("TODO: implement nonetype");
            }
            ...
            if (AP.has("-experimentAR")){
                ...
                Lir.store(mangle(id,""),getvalue(last),tid,gettype(tid)); // fix vars and blocks dicts in Lir to be function specific
                return "";
    ...
    std::vector<int> isinpool(std::vector<std::string> v,std::vector<std::string> pool){
        std::vector<int> needed;
        std::unordered_map<std::string, int> pc;
        for (auto& val : pool){
            pc[val]++;
        }
        int id = 0;
    ...
    std::vector<int> findinpool(std::vector<std::string> v, std::vector<std::string> pool,int offset=0){
        std::vector<int> tied;
        std::vector<int> addrs = {};
        for (auto& k : v){
            bool f = false;
            for (int i = 0; i != pool.size(); i++){
                if (pool[i] == k && inint(i,addrs) == false && f == false){
                    f = true;
                    addrs.push_back(i);
                    tied.push_back(i+offset);
                }
            }
        }
        return tied;
    ...
    AllocaInst* allocr(std::string name,std::string ty){
        if (AP.has("-notewarning")){
            std::cout << "Warning: With allocr manual addition to Lir.vars is required" << std::endl;
        }
        if (AP.has("-experimentAR")){
            auto cblock = blockchain[blockchain.size()-1];
            Lir.gotoblock("__allocreigon" + fnchain[fnchain.size()-1]);
            auto lt = gettype(ty);
            auto alloci = Lir.alloc(name,ty,gettype(ty));
            Lir.gotoblock(cblock);
            return alloci;

        } else {
            return Lir.alloc(name,ty,gettype(ty));

        }
    }

    std::string parseattach(){
        expect("attach");
        if (is("cmethod")){
            advance(); // cmethod
            auto id = consume().value;
            expect("to");
            auto pid = consume().value;
            ValueToValueMapTy VMap;
            
            
            Lir.fns[pid + "cmethod" + id] = CloneFunction(Lir.fns[id],VMap);
            Lir.fns[pid + "cmethod" + id]->setName(pid + "cmethod" + id);
            Lir.fns[id]->eraseFromParent();
            Lir.fns.erase(id);
            //Lir.mods[0]->getFunctionList().push_back(Lir.fns[pid + "cmethod" + id]);
            return "";
        }
        if (is("cvalue")){
            bool ext = false;
            ...
            auto pid = consume().value;
            auto e = parseexp();
            GlobalVariable* gb =new GlobalVariable(*Lir.mods[0],gettype(e[0]),false,(ext ? GlobalValue::ExternalLinkage : GlobalValue::InternalLinkage),dyn_cast<Constant>(getvalue(e[1])),pid + "cvalue" + id);
            //Lir.builder.CreateLoad(gettype(e[0]),gb,"sdneednj");
            auto vc = Var();
            vc.setallocd(gb);
            Lir.gvars[pid + "cvalue" + id] = vc;

            

            //Lir.store(pid + "cvalue" + id,getvalue(e[1]),e[0],gettype(e[0]));
            return "";
        }
    }
    std::string parseget(){
        ...
        if (is("cvalue")){
            ...
            auto lv = Lir.builder.CreateLoad(gettype(e[2]),v.gallocd,"l" + lstr);
            auto sv = lstr + std::to_string(valuects);
            valuects += 1;
            values[sv] = lv;
    ...
    std::string parsenew(){
        ...
        if (is("EnumKey")){
            expect("EnumKey");
            auto id = "enumkinst" + std::to_string(valuects);
            valuects += 1;
            expect("from");
            auto fid = consume().value;
            auto e = parseexp();
            auto eid = lookup(e[0],"enum");
            auto sv = mangle(id);
            valuects+=1;
            auto alit =  allocr(sv,e[0]);
            std::vector<Value*> fvs;
            Value* discrim;
            for (auto& k : enumfids[eid]){
                if (k.first == fid){
                    discrim = Lir.getint("i32",k.second);
                }
            }
            fvs.push_back(discrim);
            auto evar = Var();
            evar.setallocd(alit);
            Lir.vars[sv] = evar;
            //Value* p = builder.CreateStructGEP(structs[name],v,i,"field" + std::to_string(i) + "of" + as);
            //builder.CreateStore(fv[i],p);
            std::vector<std::string> evec = enumftypes[eid][fid];
            for (auto& k2: enumfvaccs[eid]){
                if (k2.first == fid){
                    int off = 1;
                    int typoff = 0;
                    for (auto& inde : k2.second){
                       auto p = Lir.builder.CreateStructGEP(gettype(eid),alit,inde,"enumfieldof" +sv + std::to_string(typoff));
                       Lir.builder.CreateStore(getvalue(e[off]),p);
                       typoff++;
                       off++;
                    }
            ...
            auto p2 = Lir.builder.CreateStructGEP(gettype(eid),alit,0,"discrimenumfieldof" +sv );
            Lir.builder.CreateStore(fvs[0],p2);
            Lir.vars[sv] = Var();
            Lir.vars[sv].setallocd(alit);
            ektofid[sv] = {eid,fid};
            values[sv] = Lir.builder.CreateLoad(gettype(eid),alit,"lenumk" + std::to_string(valuects));
            valuects += 1;
        ...
        if (is("enum")){
            ...
            std::unordered_map<std::string,std::vector<std::string>> fvs;
            std::unordered_map<std::string, std::vector<int>> fvacc;
            int discrim = 0;
            std::vector<std::string> typepool = {}; // i32 for discrim
            while (!is("endenum")){
                expect("field");
                auto fid = consume().value;
                ...
                for (int i = 0; i != e.size(); i++){
                    fvs[fid].push_back(e[i]);
                }
                fids[fid] = discrim;
                discrim++;
            }
            for (auto& kk : fvs){
                auto tpool = isinpool(kk.second,typepool);
                for (auto& attp: tpool){
                    typepool.push_back(kk.second[attp]);
                }
            }
            auto sv = mangle(id,"","enum");
            //valuects += 1;
            std::vector<Type*> tys = {gettype("i32")};
            for (auto& ty: typepool){
                tys.push_back(gettype(ty));
            }
            auto ety = Lir.initStruct(sv,tys); // now we need to tie fields to resources
            addtype(sv,ety);
            for (auto& fidk: fids){
                fvacc[fidk.first] = findinpool(fvs[fidk.first],typepool,1);
            }
            enumfids[sv] = fids;
            enumfvaccs[sv] = fvacc;
            enumftypes[sv] = fvs;
            ...
            return sv; // do other stuff later
        }
        if (is("ptr")){
            expect("ptr");
            auto exp = parseexp();
        ...
        if (is("structInstance")){
            ...
            if (AP.has("-experimentAR")){
                Lir.gotoblock("__allocreigon" + fnchain[fnchain.size()-1]);
                aiv = Lir.builder.CreateAlloca(Lir.structs[pid],nullptr,id); 
                Lir.gotoblock(blockchain[blockchain.size()-1]);
            } else {
                aiv = Lir.builder.CreateAlloca(Lir.structs[pid],nullptr,id);
            }
            auto val = Lir.initStructInstance(aiv,pid,id,fvs);
        ...
        if (is("if")){
            ...
            std::string inte2;
            if (elifs.size() != 0){
            sinte2 = "intemediary" + std::to_string(blockcts);
            blockcts += 1;
            inte2 = Lir.initblock(mangle(sinte2,"","if"),fnchain[fnchain.size()-1],false);
            }
            ...
            while (switcher != elifs.size()){
                ...
                expect("new");
                expect("block");
                auto bid = consume().value; // manually do block writes
                block1 = Lir.initblock(mangle(bid,"","elif"),fnchain[fnchain.size()-1],false);
        ...
        if (is("fn")){
            ...
            std::string sig = parsesig();
            
            if (fnname == "main"){
            
            Lir.linkext(getfnts(sig),fnname);
            Lir.initfn("MavenStart",getfnts(sig));
            
            Lir.initblock("MavenStartblk","main",true);
            
            std::vector<Value*> args = {};
            
            int asz = Lir.fns["MavenStart"]->arg_size();

            for (auto& arg: Lir.fns["main"]->args()){
            ...
            Lir.builder.CreateRet(Lir.callfn("MavenStart",args));
            
            //Lir.linkext(getfnts(parsesig()),fnname,"main");
            fnchain.push_back("MavenStart");
            } else {
            if (!ext){
            Lir.initfn(fnname,getfnts(sig));
            fnchain.push_back(fnname);
            } else {
            Lir.initextfn(fnname,getfnts(sig));
            fnchain.push_back(fnname);
            }
        ...
        if (is("block")){
            ...
            if (fblockfn.find(fnchain[fnchain.size()-1]) == fblockfn.end()){
        ...
        valuects += 1;
        if (attrs[0] == "RawString" || attrs[0] == "Character"){
        Value* v;
        ...
        } else if (attrs[0] == "void"){
            auto v = Constant::getNullValue(Lir.builder.getVoidTy());
            values[valstr] = v;
    ...
    std::string parseReturn(){
        ...
        if (blockchain.size() != 1){
            if (AP.has("-experimentAR")){
                Lir.gotoblock("__allocreigon" + fnchain[fnchain.size()-1]);
                //Lir.assign("retv" + fnchain[fnchain.size()-1],retty[fnchain[fnchain.size()-1]],gettype(retty[fnchain[fnchain.size()-1]]),getvalue(v));
                
                if (Lir.vars.find("retv" + fnchain[fnchain.size()-1]) == Lir.vars.end()){
                if (v != "void"){
                Lir.alloc("retv" + fnchain[fnchain.size()-1],retty[fnchain[fnchain.size()-1]],gettype(retty[fnchain[fnchain.size()-1]]));
                }
                }
                Lir.gotoblock(blockchain[blockchain.size()-1]);
                if (v != "void"){
                Lir.store("retv" + fnchain[fnchain.size()-1],getvalue(v),retty[fnchain[fnchain.size()-1]],gettype(retty[fnchain[fnchain.size()-1]]));
                } else {
                    voidfns.push_back(fnchain[fnchain.size()-1]);
                }

            } else {
                throw std::runtime_error("Please use the -experimentAR flag");
            }
        } else {
        if (Lir.vars.find("retv" + fnchain[fnchain.size()-1]) != Lir.vars.end()){
            if (in(fnchain[fnchain.size()-1],voidfns) == true){
                Lir.builder.CreateRetVoid();
            } else {
            Lir.endblock(Lir.builder.CreateLoad(gettype(retty[fnchain[fnchain.size()-1]]),Lir.vars["retv" + fnchain[fnchain.size()-1]].allocd));
            }
        } else {
        if (in(fnchain[fnchain.size()-1],voidfns) == true){
            Lir.builder.CreateRetVoid();
        } else {
        Lir.endblock(getvalue(v));
        }
        }
        }
        return "";
    ...
    Type* gettype(std::string t){
        std::vector<std::string> x{"struct","enum"};
    ...
    std::string inv(std::string inp){
        for (auto k = illegals.rbegin();k != illegals.rend(); k++){
            if (in(inp,k->second)){
                return k->first;
            }
        }
    ...
    std::string parsestore(){
        advance(); // store
        auto e = parseexp();
        Lir.builder.CreateStore(getvalue(e[1]),getvalue(e[0]));
        return "";
    }
    std::string parseloadas(){
        advance(); // loadas
        auto e = parseexp();
        if (values.find(e[1]) == values.end()){
        auto vn = lookup(e[1]);
        auto sv = "l" + vn + std::to_string(valuects);
        valuects += 1;
        auto v = Lir.load(vn,e[0],gettype(e[0]),sv);
        values[sv] = v;
        return sv;
        } else {
            auto sv = "loadfromv" + std::to_string(valuects);
            valuects += 1;
            auto v = Lir.builder.CreateLoad(gettype(e[0]),getvalue(e[1]),sv);
            values[sv] = v;
            return sv;
        }
    }
    std::string parseload(){
        ...
        if (in(vn,illegals[blockchain[blockchain.size()-1]])){
            ...
            Lir.gotoblock(cblock);
            flagload = Lir.builder.CreateLoad(gettype("i1"),Lir.vars["flag" + mangle(vn)].allocd,"lflag" + mangle(vn));
            Value *isAllocated = Lir.builder.CreateICmpEQ(flagload, ConstantInt::get(Type::getInt1Ty(Lir.context), 0));
    ...
    std::string parsefactor(){
        ...
        if (is("new")){
            ...
            if (AP.has("-log")){std::cout << "parsing modify: " << tokens[pos+1].value << std::endl;}
            return parsemodify();
        } else if (is("attach")){
            if (AP.has("-log")){std::cout << "parsing attach: " << tokens[pos+1].value << std::endl;}
            return parseattach();
        } else if (is("get")){
            if (AP.has("-log")){std::cout << "parsing get: " << tokens[pos+1].value << std::endl;}
            return parseget();
        } else if (is("loadas")){
            if (AP.has("-log")){std::cout << "parsing loadas: " << tokens[pos+1].value << std::endl;}
            return parseloadas();
        } else if (is("store")){
            if (AP.has("-log")){std::cout << "parsing store: " << tokens[pos+1].value << std::endl;}
            return parsestore();
        } else if (is("structload")){
            if (AP.has("-log")){std::cout << "parsing structload: " << tokens[pos+1].value << std::endl;}
            return parsestructload();
    ...
    std::string parsebitcast(){
        ...
        valuects += 1;
        Value* bc = Lir.builder.CreatePointerCast(getvalue(expr[1]),gettype(expr[0]),sbc);
        if (!AP.has("-deadcode")){

        }
        if (!bc){
            throw std::runtime_error("bad bitcast");
        }
        //std::cout <<sbc<<std::endl;
        values[sbc] = bc;
        return sbc;
    }
    std::string parsestructload(){
        expect("structload");
        auto e = parseexp();
        auto sv = "lsct" + std::to_string(valuects);
        valuects += 1;
        
        auto v = Lir.vars[lookup(e[0])].allocd;
        values[sv] = v;
        return sv;
    }
    std::string parseaccess(){
        ...
        if (is("EnumKey")){
            ...
            auto id = std::stoi(e[3]);
            auto sv = "aEK" + e[0] + std::to_string(valuects);
            valuects += 1;
            auto realid = id;
            auto v = Lir.builder.CreateStructGEP(gettype(eid),getvalue(e[2]),realid,sv);
            auto loadv = Lir.builder.CreateLoad(gettype(loadt),v,"l" + sv);
            values[sv] = loadv;
            return sv;
        ...
        if (is("structfield")){
            ...
            valuects += 1;
            std::cout << "SIM: " << structImeta[e[0]] <<std::endl;
            auto gep = Lir.builder.CreateStructGEP(gettype(structImeta[lookup(e[0])]),getvalue(e[1]),structs[structImeta[lookup(e[0])]][fid],sf);
    ...
    Value* getvalue(std::string v ){
        if (values.find(v) == values.end()){
            std::cout << "bad getvalue: " << v << "@" << currentToken().value << std::endl;
            exit(0);
        }
        return values[v];
    }
    ...
    std::string parseend(){
        ...
        if (is("fn")){
            ...
            if (fnchain.size() > 1){
                ...
                this->blockchain = fnbchain[fnchain[fnchain.size()-2]];
...
private:
    ...
    std::unordered_map<std::string,std::unordered_map<std::string, std::vector<int>>> enumfvaccs;
    std::unordered_map<std::string,std::unordered_map<std::string, std::vector<std::string>>> enumftypes;
    std::unordered_map<std::string,std::vector<std::string>> ektofid;
    std::unordered_map<std::string,std::unordered_map<std::string, int>> enumfids;
    std::vector<std::string> blockchain;
    std::vector<std::string> voidfns;
    ...
    std::unordered_map<std::string,std::string> structImeta;
    std::unordered_map<std::string,std::string> retty;
...
int main(int argc, char **argv) {
    ...
    std::cout << "Target: " << targetTriple << std::endl; native target*/
    /*
    ...
    LIR.callfn("strcat",{LIR.load("res","a100",LIR.gettypings()["u8"]),LIR.getstr("RawString","\n")});
    
    LIR.callfn("printf",{LIR.load("res","a100",LIR.gettypings()["u8"])});
    
    //auto alloc = LIR.alloc("u8",LIR.getop("+",LIR.callfn("strlen",{my_str}),LIR.callfn("strlen",{LIR.getstr("RawString","\n")})));
    ...
    auto clr = LIR.callfn("printf",{LIR.getptr(LIR.getstr("RawString","Target: " + targetTriple + "\n"))});
    LIR.callfn("println",{LIR.getptr(LIR.getstr("RawString","Hello World and"))});
    LIR.assign("pln","i32",LIR.callfn("println",{LIR.getptr(LIR.getstr("RawString","Hello LLVM"))}));
    LIR.assign("arr","a100",LIR.getstr("Character","102"),0,"Character");
    LIR.callfn("sprintf",{LIR.load("arr","a100",LIR.gettypings()["u8"]),LIR.getptr(LIR.getstr("RawString","%d")),LIR.load("pln","i32",LIR.gettypings()["i32"])});
    ...
    //Value *intValue = builder.getInt32(42);
    //Value *sum = builder.CreateAdd(intValue, builder.getInt32(10), "sum"); 
